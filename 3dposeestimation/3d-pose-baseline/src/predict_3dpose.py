...
def predict_single():
  ...
  with tf.Session(config=tf.ConfigProto(device_count=device_count)) as sess:
    ...
    for key2d in test_set_2d.keys():
      ...
      openpose_joints = np.load('src/joint_list.npy')
      test_set_2d_value_l = np.zeros([openpose_joints.shape[0]-2,2])

      test_set_2d_value_l[0] = openpose_joints[10]
      test_set_2d_value_l[1] = openpose_joints[9]
      test_set_2d_value_l[2] = openpose_joints[8]
      test_set_2d_value_l[3] = openpose_joints[11]
      test_set_2d_value_l[4] = openpose_joints[12]
      test_set_2d_value_l[5] = openpose_joints[13]
      test_set_2d_value_l[6] = (openpose_joints[8] + openpose_joints[11]) / 2
      test_set_2d_value_l[8] = openpose_joints[1]
      test_set_2d_value_l[7] = (test_set_2d_value_l[6] + test_set_2d_value_l[8]) / 2
      test_set_2d_value_l[9] = openpose_joints[0]
      test_set_2d_value_l[10] = openpose_joints[4]
      test_set_2d_value_l[11] = openpose_joints[3]
      test_set_2d_value_l[12] = openpose_joints[2]
      test_set_2d_value_l[13] = openpose_joints[5]
      test_set_2d_value_l[14] = openpose_joints[6]
      test_set_2d_value_l[15] = openpose_joints[7]

      #test_set_2d_value = np.array([128, 208, 120, 152, 128, 112, 152, 112, 168, 152, 176, 200, 140, 112, 134, 84, 128, 56, 128, 40, 64, 80, 88,72, 104, 56, 152, 48, 168, 88, 168, 112])

      #print(test_set_2d_value)
      test_set_2d_value = test_set_2d_value_l.reshape([1,32])

      #test_set_2d_value = np.array([128,208,120,152,128,112,152,112,168,152,176,200,140,112,134,84,128,56,128,40,64,80,88,72,104,56,152,48,168,88,168,112]) #test_set_2d[key2d]

      #test_set_2d_value = np.array([208,152,112,112,152,200,112,84,56,40,80,72,56,48,88,112, 128,120,128,152,168,176,140,134,128,128,64,88,104,152,168,168])

      poses = test_set_2d_value.reshape([1,16,2])
      print(poses)
      new_poses = data_utils.load_stacked_hourglass2(poses)
      print(new_poses)

      #test_set_2d_value_single = test_set_2d_value[0].reshape([1,32])
      #test_set_2d_value_single = test_set_2d_value.reshape([1, 32])
      #print(test_set_2d_value)

      test_set_2d_value_single = data_utils.normalize_data(new_poses, data_mean_2d, data_std_2d, dim_to_use_2d)
      print(test_set_2d_value_single)

      arr =  test_set_2d_value_single[(1, 'walk', 'test')]



      dp = 1.0
      _, _, poses3d = model.step(sess, arr, np.zeros([1,48]), dp, isTraining=False)
