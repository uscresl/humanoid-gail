...
from common import estimate_pose, CocoPairsRender, read_imgfile, CocoColors, draw_humans, CocoPart
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='./images/p1.jpg')
    ...
    with tf.Session(config=config) as sess:
        ...
        humans_joint = humans[0]

        pose_list = []
        for keys in humans_joint:
            joint = []
            x_coord = (humans_joint[keys][1])[0] * 256
            y_coord = (humans_joint[keys][1])[1] * 256
            print(CocoPart(humans_joint[keys][0]), ": ", x_coord,",", y_coord)
            joint.append(x_coord)
            joint.append(y_coord)
            pose_list.append(joint)

        np.save("joint_list", pose_list)
