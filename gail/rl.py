        ...
        if self.deterministic_reset:
            ...
            for joint_id in range(physics.model.njnt):
                ...
                if is_limited:
                    ...
                    if joint_type == hinge:
                        self.dm_env.physics.named.data.qpos[
                            joint_name] = 0.  # random.uniform(-np.pi, np.pi)
                        ...
                        # quat /= np.linalg.norm(quat)
                        self.dm_env.physics.named.data.qpos[joint_name][
                            3:] = quat
...
def train(num_timesteps, num_cpu, method, domain, task, noise_type, layer_norm,
          folder, video_width, video_height, plot_rewards, render_camera,
          deterministic_reset, **kwargs):
...
def main(**kwargs):
    ...
    if rank == 0:
        ...
        run_json = {
            ...
            "src_files": {
                "rl.py": "".join(open(
                    sys.argv[0], "r"))  # TODO handle case with no sys.argv[0]
            }
        }
        ...
        try:
            ...
            run_json["git"] = {
                ...
                "time":
                repo.head.object.committed_datetime.strftime(
                    "%Y-%m-%d-%H-%M-%S"),
                "author": {
