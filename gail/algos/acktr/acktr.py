...
def learn(env, pi, vf, gamma, lam, timesteps_per_batch, num_timesteps,
    ...
    inputs, loss, loss_sampled = pi.update_info
    optim = kfac.KfacOptimizer(learning_rate=stepsize, cold_lr=stepsize * (1 - 0.9), momentum=0.9, kfac_update=2,
                               epsilon=1e-2, stats_decay=0.99, async=1, cold_iter=1,
                               weight_decay_dict=pi.wd_dict, max_grad_norm=None)
    pi_var_list = []
    ...
    while True:
        ...
        paths = []
        while True:
            path = rollout(env, pi, max_pathlength, animate=(len(paths) == 0 and (i % 10 == 0) and animate),
                           obfilter=obfilter)
            paths.append(path)
            n = pathlength(path)
        ...
        # Adjust stepsize
        kl = pi.compute_kl(ob_no, oldac_dist)
        if kl > desired_kl * 2:
