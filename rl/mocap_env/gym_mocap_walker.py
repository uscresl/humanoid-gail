...
class RoboschoolMocapHumanoid(RoboschoolForwardWalkerMujocoXML):
    ...
    def __init__(self,
                 model_xml=os.path.join(
                     os.path.dirname(__file__), 'humanoid_mocap.xml')):
        RoboschoolForwardWalkerMujocoXML.__init__(
            self, model_xml, 'torso', action_dim=29, obs_dim=44, power=0.41)
    ...
    def humanoid_task(self):
        self.set_initial_orientation(
            self.TASK_WALK, yaw_center=0, yaw_random_spread=np.pi / 16)
    ...
    def apply_action(self, a):
        ...
        for i, m, power in zip(
                range(len(self.motors)), self.motors, self.motor_power):
            m.set_motor_torque(
                float(power * self.power * np.clip(a[i], -1, +1)))
